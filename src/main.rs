...
use kube::runtime::{conditions, Controller, watcher};
...
const TAINT_UNINITIALIZED: &'static str = "node.cloudprovider.kubernetes.io/uninitialized";
...
async fn reconcile_node_adoption(node: Arc<Node>, ctx: Arc<Data>) -> Result<Action, Error> {
    let client = &ctx.client;
    let deleted = node.metadata.deletion_timestamp.is_some();
    let provider_id = &node.spec.as_ref().ok_or_else(|| Error::NodeMissingSpec)?.provider_id;
    ...
    let tainted = taints.iter().any(|x| x.key.eq(TAINT_UNINITIALIZED));
    ...
    let job_pods = pods.list(&ListParams::default().labels(&("job-name=".to_string()+&job_name))).await?;
    ...
    if let Some(serde_yaml::Value::String(public_ipv4)) = metadata.get("public-ipv4") {
        ...
        if !addresses.iter().any(|x| x.type_.eq("ExternalIP")) {
            info!("patching Node {} with external IP {}", node_name, public_ipv4);
            addresses.push(NodeAddress{
                type_: "ExternalIP".to_string(),
                address: public_ipv4.clone(),
            });
            nodes.patch_status(node_name, &PatchParams::default(), &Patch::Merge(serde_json::json!({
                "status": {
                    "addresses": addresses,
                }
            }))).await?;
    ...
    if let Some(serde_yaml::Value::String(az)) = metadata.get("availability-zone") {
        if let Some(zone) = az.split("-").next() {
            if let Some(serde_yaml::Value::String(region)) = metadata.get("region") {
                ...
                if !labels.contains_key(LABEL_ZONE) && !labels.contains_key(LABEL_REGION) {
                    info!("patching Node {} with topology {} {}", node_name, region, zone);
                    nodes.patch_metadata(node_name, &PatchParams::default(), &Patch::Merge(serde_json::json!({
                        "metadata": {
                            "labels": {
                                LABEL_REGION: format!("hcloud-{}", region),
                                LABEL_ZONE: format!("hcloud-{}", zone),
                            },
                        },
                    }))).await?;
                }
    ...
    let id = metadata.get("instance-id").ok_or_else(|| Error::MetadataMissing("instance-id".to_string()))?.as_u64().unwrap();

    info!("patching Node {} with provider ID and removing taint", node_name);
    nodes.patch(node_name, &PatchParams::default(), &Patch::Merge(serde_json::json!({
        "spec": {
            "providerID": format!("hcloud://{}", id),
            "taints": taints.iter().filter(|x| x.key != TAINT_UNINITIALIZED).collect::<Vec<_>>(),
        },
    }))).await?;

    //  * set internal IPs if any private-networks
    //  * set providerID to instanceID
...
async fn main() -> anyhow::Result<()> {
    ...
    Controller::new(nodes, watcher::Config::default())
        ...
        .shutdown_on_signal()
        .run(reconcile_node_adoption, error_policy, Arc::new(Data { client }))
